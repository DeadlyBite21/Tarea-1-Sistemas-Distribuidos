#!/usr/bin/env python3
"""
Experimento LRU - 10,000 preguntas reales del dataset
Parte 1 de 3 del experimento completo
"""
...
async def save_results_to_file(results, filename="lru_results.json"):
...
async def test_lru_cache_10k():
    """Experimento LRU con 10,000 preguntas reales"""
    ...
    print(f"\n{'='*80}")
    print(f"🧪 EXPERIMENTO LRU - 10,000 PREGUNTAS REALES")
    print(f"{'='*80}")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print(f"\n🚀 INICIANDO PROCESAMIENTO DE 10,000 PREGUNTAS REALES (LRU)")
        ...
        try:
            async with session.get(stats_url) as response:
                if response.status == 200:
                    ...
                    print(f"\n{'='*80}")
                    print(f"📈 RESULTADOS FINALES - LRU (10,000 PREGUNTAS REALES)")
                    print(f"{'='*80}")
                    ...
                    print(f"    • Eficiencia del LRU cache: {final_stats.get('cache_hits', 0)/max(final_stats.get('total_requests', 1), 1)*100:.1f}%")
                    
                    # Guardar resultados
                    await save_results_to_file(final_stats, "lru_results.json")
                    
                    print(f"\n🎯 EXPERIMENTO LRU COMPLETADO")
                    print(f"📁 Resultados guardados en '../results/lru_results.json'")
...
def main():
    print("🚀 EXPERIMENTO LRU - PARTE 1 DE 3")
    ...
    print(f"  1. Ejecuta este script (LRU)")
    print(f"  2. Compañero 2 ejecuta 'experiment_lfu.py' (LFU)")
    ...
    asyncio.run(test_lru_cache_10k())
